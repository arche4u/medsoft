...
from app.modules.compliance.dev.design.model import DesignElement
...
async def create_change_request(
    ...
    cr = ChangeRequest(
        ...
        modifies_released_software=body.modifies_released_software,
    )
    db.add(cr)
    await db.flush()
...
@router.put("/requests/{cr_id}", response_model=ChangeRequestRead)
async def update_change_request(
    ...
    if not cr:
        raise HTTPException(404, "ChangeRequest not found")
    if cr.status == ChangeRequestState.IMPLEMENTED:
        raise HTTPException(400, "Cannot edit a CR after IMPLEMENTED — open a new CR for further changes")
    for field, value in body.model_dump(exclude_unset=True).items():
        setattr(cr, field, value)
    await audit(db, "ChangeRequest", cr.id, AuditAction.UPDATE, current_user.user_id,
                "Post-release impact analysis updated" if (body.effect_on_organization or
                body.effect_on_released_software or body.effect_on_interfacing_systems) else None)
    await db.commit()
    await db.refresh(cr)
    return cr
...
async def transition_change_request(
    ...
    if body.new_status == ChangeRequestState.APPROVED:
        ...
        # IEC 62304 §6.2.3 — CRs that modify released software require explicit
        # analysis of effect on (a) the organization, (b) released software,
        # and (c) interfacing systems before §6.2.4 approval.
        if cr.modifies_released_software:
            missing = [
                ("effect_on_organization", cr.effect_on_organization),
                ("effect_on_released_software", cr.effect_on_released_software),
                ("effect_on_interfacing_systems", cr.effect_on_interfacing_systems),
            ]
            empty = [name for name, val in missing if not (val and val.strip())]
            if empty:
                raise HTTPException(
                    400,
                    "IEC 62304 §6.2.3 requires post-release impact analysis before "
                    f"approving a CR that modifies released software. Missing: {', '.join(empty)}.",
                )
...
async def _auto_populate_impacts(db: AsyncSession, cr: ChangeRequest) -> None:
    ...
    from app.modules.compliance.dev.requirements.model import RequirementCategory
