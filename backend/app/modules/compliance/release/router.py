...
from app.modules.compliance.dev.system_testing.model import SystemTestResult
...
async def transition_release(
    ...
    if body.new_status == ReleaseStatus.RELEASED:
        ...
        from app.modules.compliance.dev.system_testing.router import _compute_readiness
        ...
        from app.modules.compliance.problems.capa.router import capa_release_check as _capa_check
        ...
        from app.modules.compliance.config.config_mgmt.router import release_check as _cm_check
        ...
        from app.modules.compliance.dev.integration_tests.router import get_coverage as _itc_coverage
...
@router.patch("/releases/{release_id}/notify", response_model=ReleaseRead)
async def record_notification(
    release_id: uuid.UUID,
    body: ReleaseNotificationUpdate,
    db: AsyncSession = Depends(get_db),
    current_user: TokenData = Depends(require_permission("PUBLISH_RELEASE")),
):
    """Record that users or regulators have been notified about the changes
    in this release, per §6.2.5. Audit trail captures who recorded it and when."""
    rel = (await db.execute(select(Release).where(Release.id == release_id))).scalar_one_or_none()
    if not rel:
        raise HTTPException(404, "Release not found")
    ...
    await audit(db, "Release", rel.id, AuditAction.UPDATE, current_user.user_id,
                f"§6.2.5 {audience} notification recorded")
    await db.commit()
    await db.refresh(rel)
    return rel
