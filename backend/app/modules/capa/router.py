...
async def create_problem(body: ProblemReportCreate, db: AsyncSession = Depends(get_db)):
...
async def update_problem(problem_id: str, body: ProblemReportUpdate, db: AsyncSession = Depends(get_db)):
    p = (await db.execute(select(ProblemReport).where(ProblemReport.id == uuid.UUID(problem_id)))).scalar_one_or_none()
    if not p:
        raise HTTPException(404, "Problem report not found")
    if p.status == "CLOSED":
...
async def transition_problem(problem_id: str, body: ProblemStatusTransition, db: AsyncSession = Depends(get_db)):
    p = (await db.execute(select(ProblemReport).where(ProblemReport.id == uuid.UUID(problem_id)))).scalar_one_or_none()
    if not p:
        raise HTTPException(404, "Problem report not found")
...
async def add_link(problem_id: str, body: ProblemLinkCreate, db: AsyncSession = Depends(get_db)):
    p = (await db.execute(select(ProblemReport).where(ProblemReport.id == uuid.UUID(problem_id)))).scalar_one_or_none()
    if not p:
        raise HTTPException(404, "Problem report not found")
...
async def add_root_cause(problem_id: str, body: RootCauseCreate, db: AsyncSession = Depends(get_db)):
    p = (await db.execute(select(ProblemReport).where(ProblemReport.id == uuid.UUID(problem_id)))).scalar_one_or_none()
    if not p:
        raise HTTPException(404, "Problem report not found")
    if p.status == "CLOSED":
...
async def delete_root_cause(rc_id: str, db: AsyncSession = Depends(get_db)):
...
async def create_capa(problem_id: str, body: CAPACreate, db: AsyncSession = Depends(get_db)):
    p = (await db.execute(select(ProblemReport).where(ProblemReport.id == uuid.UUID(problem_id)))).scalar_one_or_none()
    if not p:
        raise HTTPException(404, "Problem report not found")
    if p.status == "CLOSED":
...
async def update_capa(capa_id: str, body: CAPAUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_capa(capa_id: str, db: AsyncSession = Depends(get_db)):
    capa = (await db.execute(select(CAPA).where(CAPA.id == uuid.UUID(capa_id)))).scalar_one_or_none()
    if not capa:
        raise HTTPException(404, "CAPA not found")
...
async def add_verification(capa_id: str, body: CAPAVerificationCreate, db: AsyncSession = Depends(get_db)):
    capa = (await db.execute(select(CAPA).where(CAPA.id == uuid.UUID(capa_id)))).scalar_one_or_none()
    if not capa:
        raise HTTPException(404, "CAPA not found")
...
async def delete_verification(v_id: str, db: AsyncSession = Depends(get_db)):
...
async def list_maintenance(project_id: str, db: AsyncSession = Depends(get_db)):
...
async def create_maintenance(body: MaintenanceRecordCreate, db: AsyncSession = Depends(get_db)):
...
async def update_maintenance(rec_id: str, body: MaintenanceRecordUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_maintenance(rec_id: str, db: AsyncSession = Depends(get_db)):
    rec = (await db.execute(select(MaintenanceRecord).where(MaintenanceRecord.id == uuid.UUID(rec_id)))).scalar_one_or_none()
    if not rec:
        raise HTTPException(404, "Maintenance record not found")
    await db.delete(rec)
...
async def capa_release_check(project_id: str, db: AsyncSession = Depends(get_db)):
