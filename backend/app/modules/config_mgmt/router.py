...
async def create_item(body: ConfigItemCreate, db: AsyncSession = Depends(get_db)):
    ci = CMConfigItem(**body.model_dump())
    db.add(ci)
    await db.commit()
    await db.refresh(ci)
    # seed initial version history entry
...
async def update_item(item_id: str, body: ConfigItemUpdate, db: AsyncSession = Depends(get_db)):
    ci = (await db.execute(select(CMConfigItem).where(CMConfigItem.id == uuid.UUID(item_id)))).scalar_one_or_none()
    if not ci:
        raise HTTPException(404, "Config item not found")
...
async def create_new_version(item_id: str, body: ConfigItemNewVersion, db: AsyncSession = Depends(get_db)):
    ci = (await db.execute(select(CMConfigItem).where(CMConfigItem.id == uuid.UUID(item_id)))).scalar_one_or_none()
    if not ci:
        raise HTTPException(404, "Config item not found")
...
async def set_item_status(item_id: str, status: str, db: AsyncSession = Depends(get_db)):
...
async def create_baseline(body: BaselineCreate, db: AsyncSession = Depends(get_db)):
    bl = CMBaseline(
...
async def add_item_to_baseline(baseline_id: str, item_id: str, db: AsyncSession = Depends(get_db)):
    bl = (await db.execute(select(CMBaseline).where(CMBaseline.id == uuid.UUID(baseline_id)))).scalar_one_or_none()
    if not bl:
        raise HTTPException(404, "Baseline not found")
    if bl.is_released:
...
async def remove_item_from_baseline(baseline_id: str, item_id: str, db: AsyncSession = Depends(get_db)):
    bl = (await db.execute(select(CMBaseline).where(CMBaseline.id == uuid.UUID(baseline_id)))).scalar_one_or_none()
...
async def release_baseline(baseline_id: str, db: AsyncSession = Depends(get_db)):
    bl = (await db.execute(select(CMBaseline).where(CMBaseline.id == uuid.UUID(baseline_id)))).scalar_one_or_none()
    if not bl:
        raise HTTPException(404, "Baseline not found")
    if bl.is_released:
...
async def get_change(cr_id: str, db: AsyncSession = Depends(get_db)):
...
async def create_change(body: ChangeRequestCreate, db: AsyncSession = Depends(get_db)):
...
async def update_change(cr_id: str, body: ChangeRequestUpdate, db: AsyncSession = Depends(get_db)):
    cr = (await db.execute(select(CMChangeRequest).where(CMChangeRequest.id == uuid.UUID(cr_id)))).scalar_one_or_none()
    if not cr:
        raise HTTPException(404, "Change request not found")
...
async def transition_change(cr_id: str, body: ChangeRequestStatusTransition, db: AsyncSession = Depends(get_db)):
    cr = (await db.execute(select(CMChangeRequest).where(CMChangeRequest.id == uuid.UUID(cr_id)))).scalar_one_or_none()
    if not cr:
        raise HTTPException(404, "Change request not found")
...
async def delete_change(cr_id: str, db: AsyncSession = Depends(get_db)):
    cr = (await db.execute(select(CMChangeRequest).where(CMChangeRequest.id == uuid.UUID(cr_id)))).scalar_one_or_none()
    if not cr:
        raise HTTPException(404, "Change request not found")
...
async def add_impact(cr_id: str, body: ChangeImpactCreate, db: AsyncSession = Depends(get_db)):
    cr = (await db.execute(select(CMChangeRequest).where(CMChangeRequest.id == uuid.UUID(cr_id)))).scalar_one_or_none()
    if not cr:
        raise HTTPException(404, "Change request not found")
    if cr.status in ("CLOSED", "REJECTED"):
...
async def update_impact(impact_id: str, body: ChangeImpactUpdate, db: AsyncSession = Depends(get_db)):
...
async def release_check(project_id: str, db: AsyncSession = Depends(get_db)):
